...
# ARM: Generic hardware scripts (most should be safe, detect at runtime)
...
# ARM: Skip NVIDIA (different drivers on ARM)
# run_logged $OMARCHY_INSTALL/config/hardware/nvidia.sh
run_logged $OMARCHY_INSTALL/config/hardware/vulkan.sh

# ARM: Skip all Intel-specific scripts
...
# ARM: Skip all ASUS-specific scripts
...
# ARM: Skip Framework-specific scripts
...
# ARM: Skip Apple-specific scripts
...
# ARM: Skip Lenovo-specific scripts
# run_logged $OMARCHY_INSTALL/config/hardware/lenovo/fix-yoga-pro7-bass-speakers.sh

# ARM: Skip specific hardware fixes that are x86-only
